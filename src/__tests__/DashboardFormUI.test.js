...
describe("Given I am connected as an Admin and I am on Dashboard Page", () => {
    ...
    describe("When bill data with null fileName is passed to DashboardUI", () => {
        test('Then, it should display "Aucun fichier disponible"', () => {
            const billWithNullFileName = {
                ...bill,
                fileName: "null",
            };
            const html = DashboardFormUI(billWithNullFileName);
            document.body.innerHTML = html;
            expect(screen.getByText("Aucun fichier disponible")).toBeTruthy();
        });
    });
});
